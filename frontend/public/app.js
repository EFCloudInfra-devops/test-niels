...
function render() {
  ...
  for (const member of [0,1]) {
    ...
    pane.className = 'pane';
    pane.innerHTML = `<h2>VC-lid ${member}</h2>`;
...
form.addEventListener('submit', async (e) => {
  e.preventDefault();
  const cfg = collectConfig(currentPort);
  ...
  if (data.ok) {
    ...
    alert('Commit mislukt: ' + data.error);
  }
});

async function validate(cfg) {
  const res = await fetch(`${API}/validate`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(cfg)});
  const data = await res.json();
  if (!data.ok) {
    alert('Validatiefouten: ' + data.errors.join('; '));
  }
  return data.ok;
}
